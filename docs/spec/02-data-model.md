...
- **completions**: id · instance_id · chore_id · child_id · household_id · chore_date · completed_at (UTC) · device_id · photo_key · status (accepted|pending_photo|rejected|undone) · rejected_by · rejected_at · created_at. Append-only: the row is written once and afterwards only its status moves, with one exception — a `pending_photo` row's `photo_key` is filled in once when the upload lands (ADR-0017), which is the tap's own write completing rather than a second edit of a settled row. It never changes again, and no other column moves. `undone` is the child's own same-day undo — the row stays and stops counting, so a later parent rejection can see it was already undone. Re-completing after an undo writes a **new** completion. Completion of a deleted chore is accepted and paid.
